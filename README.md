...
El análisis sintáctico —o *parsing*— constituye la segunda fase dentro de la arquitectura de un compilador. Su propósito consiste en recibir la secuencia de componentes léxicos (*tokens*) producida por el analizador léxico y determinar si dicha secuencia respeta las reglas estructurales definidas por una **Gramática Libre de Contexto (GLC)**. Más allá de validar la sintaxis, el analizador construye una representación jerárquica en forma de árbol que servirá como base para las etapas posteriores de análisis semántico y generación de código.

A lo largo de este taller se abordan tres problemas fundamentales del diseño sintáctico:

1. **Implementación de una gramática clásica de expresiones.** Se construye un analizador sintáctico en Python empleando ANTLR 4 y el patrón de diseño *Visitor*, evaluando con rigor las expresiones aceptadas y rechazadas según la especificación formal propuesta.
2. **Contraste entre formas de árbol (Parse Tree vs. AST).** Se estudia la transformación de un árbol de derivación concreto hacia un árbol de sintaxis abstracta, analizando el impacto de la precedencia de operadores, la asociatividad y el uso de paréntesis sobre la forma del árbol resultante.
3. **Identificación y demostración de ambigüedad gramatical.** Se demuestra, tanto de forma práctica como matemática, cómo una gramática carente de niveles de precedencia produce múltiples árboles para una misma entrada, dando lugar a resultados semánticos incompatibles.

---

## Estructura del Proyecto

El repositorio se organiza de manera modular en tres carpetas independientes, acompañadas por herramientas de automatización para su compilación y ejecución:
...
- **Nivel $E$ (Expresión):** maneja la suma. Al ser el nivel superior, posee la menor precedencia.
- **Nivel $T$ (Término):** maneja la multiplicación. Al estar subordinado a $E$, posee mayor precedencia.
...
- **`main.py`:** configura un `SyntaxErrorCollector` que sustituye los manejadores por defecto de ANTLR con el fin de capturar anomalías léxicas y sintácticas sin interrumpir la ejecución. Si la cadena cumple con la gramática, retorna **`ACEPTADA`**; en caso contrario, emite **`RECHAZADA`** detallando la causa exacta del fallo.
...
2. **Observación sobre el operador de resta (`-`):**
   ...
   - En consecuencia, el analizador riguroso reporta **`RECHAZADA`** para cualquier expresión que contenga `-`, dado que el símbolo no está contemplado en la regla de producción formal. Para que dichas expresiones fuesen aceptadas, la regla requeriría extenderse a:
     $$E \to E ('+' \mid '-') T \mid T$$
...
Existe una distinción fundamental entre el árbol que genera el analizador y el árbol que utiliza el compilador:

- **Parse Tree (Árbol Sintáctico Concreto — Diapositiva 12):** refleja de forma exhaustiva cada paso de la derivación formal. Incluye todos los símbolos no terminales ($E, T, F$) y los terminales de puntuación (paréntesis). Resulta indispensable durante el *parsing*, pero introduce un costo significativo en memoria y complejidad.
- **AST (Árbol de Sintaxis Abstracta — Diapositiva 13):** constituye una representación sintética y compacta que descarta el ruido sintáctico. Los operadores se posicionan como nodos internos y los operandos como hojas.
...
- **Demostración de unicidad:** para esta cadena, la gramática produce **exactamente un único AST**. Resulta imposible que el operador `*` quede en la raíz sin paréntesis, lo cual confirma que la gramática de la diapositiva 12 no es ambigua.

**Caso B — Precedencia forzada por paréntesis (`(3 + 4) * 5`)**

Los paréntesis actúan como modificadores de jerarquía: fuerzan a que la suma se resuelva dentro del factor $F$, subordinando el operador `+` bajo el operador `*`. Cabe destacar que los caracteres `(` y `)` **no se incorporan como nodos en el AST**, pues su función ordenadora queda reflejada directamente en la topología del árbol.
...
- **Forma del AST:** `(+ (+ 3 4) 5)` → el subárbol izquierdo contiene la primera suma `3 + 4`, y el resultado se suma posteriormente con `5`.
...
- **Optimización:** reducción del **66.7%** en la cantidad de nodos en memoria, lo cual demuestra por qué las fases posteriores del compilador (análisis semántico y generación de código) operan exclusivamente sobre el AST.
...
Dado que una computadora no puede asumir de manera determinista cuál de los dos significados pretendía el programador, una gramática ambigua resulta inaceptable para la construcción de lenguajes de programación. Para eliminar la ambigüedad, es obligatorio:
...
Para eliminar todos los archivos autogenerados por ANTLR (`*.tokens`, `*.interp`, archivos `.py` derivados) y las cachés `__pycache__`:
